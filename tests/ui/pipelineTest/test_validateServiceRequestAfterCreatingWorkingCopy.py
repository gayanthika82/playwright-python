...
@pytest.mark.test_case_ids([88159])
def test_validateServiceRequestAfterCreatingWorkingCopy_step1(page_setup) -> None:
    logging.info("test_validateServiceRequestAfterCreatingWorkingCopy_step1")
    page = page_setup
    global title
    global editTitle
...
@pytest.mark.test_case_ids([88159])
@pytest.mark.dependency(depends=["test_validateServiceRequestAfterCreatingWorkingCopy_step1"])
def test_validateServiceRequestAfterCreatingWorkingCopy_step2(page_setup) -> None:
    logging.info("Starting test_validateServiceRequestAfterCreatingWorkingCopy_step2")
    page = page_setup
    common_utils = CommonUtils(page)
...
@pytest.mark.test_case_ids([88159])
@pytest.mark.dependency(depends=["test_validateServiceRequestAfterCreatingWorkingCopy_step2"])
def test_validateServiceRequestAfterCreatingWorkingCopy_step3(page_setup) -> None:
    logging.info("Starting test_validateServiceRequestAfterCreatingWorkingCopy_step3")
    page = page_setup
    common_utils = CommonUtils(page)
    # Get the userId and username from environment variables
    userId_reviewer_a = os.getenv("USERNAME_REVIEWER_A")
