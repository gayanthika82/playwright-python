...
@pytest.mark.test_case_ids([88061,88131])
def test_cancelWorkingCopy(page_setup) -> None:
    logging.info("Starting test_cancel_workingCopy")
    page = page_setup
    
    # Create record title
    common_utils = CommonUtils(page)
    title = "CancelWorkingCopy"
    title = common_utils.createCatalogueRecordTitle(title)
    editTitle = "Edit Working Copy Title"
    ...
    page.locator(catalogue_objects.WORKING_COPY_BUTTON).click()
    ...
    #Cancel working copy record    
    page.once("dialog", lambda dialog: dialog.accept())
    page.locator(catalogue_objects.CANCEL_WORKING_COPY_BUTTON).click()
    logging.info("Clicking cancel working copy button")
    expect(page.get_by_role("alert")).to_contain_text(editTitle+" removed")
    logging.info("Working copy record is removed successfully")
        
    #Validate no more working copy
    time.sleep(5)
    common_utils.searchRecordInEditorBoard(editTitle)
    time.sleep(5)    
    #expect(page.locator("body")).to_contain_text("No results found!")
    logging.info("No results found for original record after cancelling working copy")
